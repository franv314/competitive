...
struct SegTree {
    struct Node {
        ...
        Node(int b, int e, long long score, Node *l, Node *r) :
            val(0), nb(b), ne(e), score(score), ans(0), lc(l), rc(r) { }
    } *root;

    Node* build(int nb, int ne, long long scores[]) {
...
public:
    ...
    long long query() { return root->ans; }

    SegTree(vector<long long> scores) {
...
int main() {
    int n; scanf("%d", &n);
    vector<tuple<long long, int, long long, long long>> things;
    ...
    long long old = compress.begin()->first;
